 ...
 */
void dfs_recur(int vertex, std::vector<int> adj[], std::vector<bool>& visited, std::vector<int>& result) {
    ...
    for (int neighbor : adj[vertex]) {
        if (!visited[neighbor]) {
            dfs_recur(neighbor, adj, visited, result);
        }
    }
}
...
std::vector<int> dfs_of_graph(int V, std::vector<int> adj[]) {
    ...
    dfs_recur(0, adj, visited, result);
